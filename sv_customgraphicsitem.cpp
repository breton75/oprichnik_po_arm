...
//  _data.volume= qreal(qrand()) / RAND_MAX;
  ...
  QString s = QString("%1 т.").arg(_data.weight, 0, 'f', 2);
  path.addText(QPointF(50 - fm.width(s)/2, 50 - fm.height()), fnt, s);

  s = QString("%1 куб.м").arg(_data.volume, 0, 'f', 2);
...
void SvConsumerGraphicsItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *widget)
{
  Q_UNUSED(widget);
...
//=======
  ...
  //QString strval = QString::number( _data.spend_v, 'g', 4);
  //QString strval = QString("%1 л/сек").arg(_data.spend_v, 'f', 3);
  QString strval = QString::number(_data.spend_v, 'f', 3).append(" л/мин");
  //path.addText(QPointF(50 - fm.width(strval)/2, 52 - fm.height()/2), fnt, strval);0
  path.addText(QPointF(50 - fm.width(strval)/2, 52), fnt, strval);
